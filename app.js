...
forecasts.forEach(forecast => { const date = forecast.dt_txt.split(' ')[0]; if (!grouped[date]) grouped[date] = []; grouped[date].push(forecast); });

const result = Object.entries(grouped) .slice(0, 3) .map(([date, entries]) => { const pressures = entries.map(e => e.main.pressure); const min = Math.min(...pressures); const max = Math.max(...pressures); const avg = pressures.reduce((a, b) => a + b, 0) / pressures.length;
...
function displayForecasts(forecasts) { const container = document.getElementById('forecast'); container.innerHTML = '';

forecasts.forEach(day => { const card = document.createElement('div'); card.style.backgroundColor = day.color; card.style.padding = '10px'; card.style.margin = '10px'; card.style.borderRadius = '10px'; card.style.color = '#000'; card.style.fontFamily = 'Arial';
